...
contract XECO is IBEP20, OwnableUpgradeable {
    ...
    function initialize() initializer public {
        ...
        _minTokenBalance = 1000000 * 10**18;
    ...
    function InitializeContract(address cOwner, address devWallet, address marketingWallet, address proxyWallet) public onlyOwner{
        if(_isIntialized)
        {
            revert ("Already Intialized");
        }
        _devWallet = devWallet;
        _marketingWallet = marketingWallet;
        _proxyAddress = proxyWallet;
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        if(_isTradeable||_isExcludedFromFee[_msgSender()])
        {
    ...
    function setPercentageForDevWallet(uint256 percent) external onlyOwner{
        _percentageOfLiquidityForDev = percent;
    }

    function setPercentageForMarketingwallet(uint256 percent) external onlyOwner{
        _percentageforOperationsAndMarket = percent;
    }

    function setLiquidityPercentage(uint256 percent) external onlyOwner{
        _percentageforLiquidityTax = percent;
    }

    function getLiquidityPercentage() external view returns(uint256) {
        return _percentageforLiquidityTax;
    }
    function getMarketingPercentage () external view returns(uint256){
        return _percentageforOperationsAndMarket;
    }
    function getDevWalletPercentage() external view returns(uint256){
        return _percentageOfLiquidityForDev;
    }
    ...
    function _transfer(
        ...
        */
        ...
        uint256 contractTokenBalance = balanceOf(_proxyAddress);
